...
git pull это одна из 4 удаленных операций в Git. Без запуска git pullваш локальный репозиторий никогда не будет обновляться изменениями с удаленного. git pullследует использовать каждый день, когда вы взаимодействуете с репозиторием с помощью удаленного, как минимум. Вот почему git pullэто одна из наиболее часто используемых команд Git.
